        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-green-600 dark:text-green-400">{{ money(row.actual_cost) }}</span>
              <span
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ money(tooltipData.input_cost) }}</span>
            </div>
            <div v-if="tooltipData && hasImageInputCost(tooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-fuchsia-300">{{ money(tooltipData.image_input_cost) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.outputCost') }}</span>
              <span class="font-medium text-white">{{ money(tooltipData.output_cost) }}</span>
            </div>
            <div v-if="tooltipData && hasImageOutputCost(tooltipData)" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-pink-300">{{ money(tooltipData.image_output_cost) }}</span>
            </div>
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              <div v-if="tooltipData && textInputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ tokenPricePerMillion(tooltipData.input_cost, textInputTokens(tooltipData)) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-fuchsia-300">{{ tokenPricePerMillion(tooltipData.image_input_cost ?? 0, tooltipData.image_input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && tooltipData.output_cost > 0 && textOutputTokens(tooltipData) > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-violet-300">{{ tokenPricePerMillion(tooltipData.output_cost, textOutputTokens(tooltipData)) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-pink-300">{{ tokenPricePerMillion(tooltipData.image_output_cost ?? 0, tooltipData.image_output_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
            </template>
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ money(imageUnitPrice(tooltipData)) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">{{ money(tooltipData.total_cost || 0) }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-sky-300">{{ money(tooltipData?.total_cost || 0) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheCreationCost') }}</span>
              <span class="font-medium text-white">{{ money(tooltipData.cache_creation_cost) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ money(tooltipData.cache_read_cost) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-green-400">{{ money(tooltipData?.actual_cost ?? 0) }}</span>
          </div>
...
import { formatDateTime, formatReasoningEffort, formatCostDisplay } from '@/utils/format'
...
interface Props {
  ...
  flat?: boolean
  /**
   * 用户端展示层汇率（1 USD → CNY）。传正数则成本按 ¥ 显示，缺省/非正数按 $ 显示（管理端默认）。
   */
...
// 成本金额换算：管理端不传 displayCnyRate 保持 $；用户端传汇率显示 ¥。
// 单笔明细金额很小，保留 6 位小数（与原 toFixed(6) 一致），避免换算后精度丢失。
const money = (usd: number | null | undefined) => formatCostDisplay(usd, props.displayCnyRate, 6)

// 每百万 token 单价换算：与 money 场景一致（用户端 ¥ / 管理端 $），保留 4 位小数（与原 formatTokenPricePerMillion 默认一致）。
const tokenPricePerMillion = (cost: number | null | undefined, tokens: number | null | undefined): string => {
  const perMillion = calculateTokenPricePerMillion(cost, tokens)
  if (perMillion == null) return '-'
  return formatCostDisplay(perMillion, props.displayCnyRate, 4)
}

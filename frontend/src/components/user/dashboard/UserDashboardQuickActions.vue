<template>
  <div class="card">
    ...
    <div class="space-y-3 p-4">
      ...
      <button @click="router.push('/purchase')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-primary-100 transition-transform group-hover:scale-105 dark:bg-primary-900/30">
          <Icon name="creditCard" size="lg" class="text-primary-600 dark:text-primary-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('longdao.dashboard.planCenter') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('longdao.dashboard.planCenterDesc') }}</p>
        </div>
        <Icon name="chevronRight" size="md" class="text-gray-400 transition-colors group-hover:text-primary-500 dark:text-dark-500" />
      </button>

      <button @click="router.push('/recharge')" class="group flex w-full items-center gap-4 rounded-xl bg-gray-50 p-4 text-left transition-all duration-200 hover:bg-gray-100 dark:bg-dark-800/50 dark:hover:bg-dark-800">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-rose-100 transition-transform group-hover:scale-105 dark:bg-rose-900/30">
          <Icon name="dollar" size="lg" class="text-rose-600 dark:text-rose-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('longdao.dashboard.balanceRecharge') }}</p>
          <p class="text-xs text-gray-500 dark:text-dark-400">{{ t('longdao.dashboard.balanceRechargeDesc') }}</p>
        </div>
        <Icon name="chevronRight" size="md" class="text-gray-400 transition-colors group-hover:text-rose-500 dark:text-dark-500" />
      </button>
